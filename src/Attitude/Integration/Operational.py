...
def Run_L1AtoL1B_perday(date=None, sat=SatID.D, imu_disable=None):
    ...
    SCA.produce_combine_first(mission=Mission.GRACE_FO, isCombined=True)
...
def Run_resample_perday(date=None, sat=SatID.D):
    # sat = SatID.C
    mission = Mission.GRACE_FO
    # date = '2019-01-02'

    rs = ReSample(mission=mission, date=date, sat=sat)
    rs.run()
    pass
...
def Run_complete(begin='2019-01-01', end='2019-01-31', sat=SatID.D):
    ...
    for day in daylist:
        ...
        print('****************************************************\n')
        isEmpty = Run_L1AtoL1B_perday(date=day, sat=sat, imu_disable=IMUID.No_4)
        if isEmpty:
            print('SCA-1A is an empty file, so that this day is skipped.')
            continue
        ...
        if isEmpty:
            ...
            continue
        Run_resample_perday(date=day, sat=sat)
        # los = LOS(mission=Mission.GRACE_FO, date=day).configDir('../data/GRACE_FO/RL04/L1B')
        # los.getLOS(DataDir='../result/product/GRACE_FO/RL04/L1B/')
        # Run_reformat(date=day)
...
if __name__ == '__main__':
    Run_complete(begin='2021-01-01', end='2021-09-30', sat=SatID.D)
    # Run_reformat()
