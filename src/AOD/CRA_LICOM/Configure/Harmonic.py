...
from src.AOD.CRA_LICOM.Configure.LoveNumber import LoveNumber
from src.AOD.CRA_LICOM.Configure.RefEllipsoid import RefEllipsoid
from src.AOD.CRA_LICOM.Configure.Setting import SynthesisType, EllipsoidType, LoveNumberType, Constants, HarAnalysisType
...
class Harmonic:
    ...
    def __init__(self, Ln: LoveNumber):
        self._Ln = Ln
    ...
    def analysis(self, Nmax: int, Gqij, lat, lon, PnmMat, kind=HarAnalysisType.InnerIntegral):
        """
        ...
        """
        Gqij = np.array(Gqij)
        assert len(np.shape(Gqij)) in [3, 2]
        ...
        if kind is not HarAnalysisType.InnerIntegral:
            '''in this case, it is a series'''
            if np.shape(Cnms)[0] == 1:
                Cnms = Cnms[0]
                Snms = Snms[0]
            return Cnms, Snms
        else:
            '''in this case, it is an epoch'''
            assert len(Gqij[:, 0, 0]) == Nmax + 1
            Cqlm, Sqlm = np.zeros((Nmax + 1, Nmax + 1)), np.zeros((Nmax + 1, Nmax + 1))
            for l in range(Nmax + 1):
                Cqlm[l, :l + 1] = Cnms[l, l, :l + 1]
                Sqlm[l, :l + 1] = Snms[l, l, :l + 1]
            return Cqlm, Sqlm

    def analysis_smaller_meomory_innerIntegral(self, Nmax: int, adr, pdg, lat, lon, PnmMat):
        """
        A simpson-type quadrature method.
        lat,lon, I and Pnm should keep accordance.
        The output is SH coefficients in terms of geo-potential.
        ...
        factorSH = self._factorHarAnalysis(Nmax=Nmax, kind=HarAnalysisType.InnerIntegral)
        ...
        for l in range(Nmax + 1):
            ini = ini * adr
            Gqij = (pdg.T[:, None, :] @ ini.T[:, :, None]).reshape((len(lat), len(lon)))

            Am = np.einsum('ij,jm->im', Gqij, co, optimize='greedy') * factor1
            Bm = np.einsum('ij,jm->im', Gqij, so, optimize='greedy') * factor1

            Cnms = np.einsum('im,ilm,i->lm', Am, PnmMat, np.sin(colat_rad), optimize='greedy') * factor2
            Snms = np.einsum('im,ilm,i->lm', Bm, PnmMat, np.sin(colat_rad), optimize='greedy') * factor2

            Cnms = np.einsum('lm,l->lm', Cnms, factorSH, optimize='greedy')
            Snms = np.einsum('lm,l->lm', Snms, factorSH, optimize='greedy')

            Cqlm[l, :l + 1] = Cnms[l, :l + 1]
            Sqlm[l, :l + 1] = Snms[l, :l + 1]

        return Cqlm, Sqlm

    def synthesis(self, Cqlm, Sqlm, Nmax, lat, lon, PnmMat, kind=SynthesisType.Pressure):
        """
        ...
        if np.ndim(Cnms) == 2:
            ...
            flag = 1
            Cnms = GeoMathKit.CS_1dTo2d(Cnms)
            Snms = GeoMathKit.CS_1dTo2d(Snms)
            Cnms = Cnms[None, :, :]
            Snms = Snms[None, :, :]
        ...
        Fout = Am @ co + Bm @ so
        return np.array(Fout) if flag == 0 else np.array(Fout[0])
...
class Harmonic2:
    ...
    def job(self, n):
        ...
        for m in range(n + 1):
            Am[m] = factor1 * np.array(I_new @ np.cos(m * phi).T).flatten()
            Bm[m] = factor1 * np.array(I_new @ np.sin(m * phi).T).flatten()
