...
from LoadFields import LoadFields, DataType
from Harmonic import Harmonic, LoveNumber, LoveNumberType, HarAnalysisType
from Format import FormatWrite, CnmSnm
from SurPres2CS import SurPres2CS, RefEllipsoid, EllipsoidType
...
class TideFit:
    ...
    def __init__(self):
        self._timeEpoch = ["00:00:00", "06:00:00", "12:00:00", "18:00:00"]
        ...
        for indexByLat in range(Nlat):
            ...
            for date in self._daylist:
                for time in self._timeEpoch:
                    ...
                    # ld.setTime_for_tide(date=str(date),time=str(time))
            ...
            for index in np.arange(NLon):
                ...
                # '''least-square fit to derive the tides'''
                # # popt, pcov = curve_fit(self.__BTS1S2, xdata, sf)
                # popt, pcov = curve_fit(self.__all, xdata, sf)
                #
                # # tideS1.append(popt[2:4])
                # # tideS2.append(popt[4:6])
                # tide.append(popt[2:])
...
def demo2():
    ...
    tf.setDataDir(dataDirIn='../data/CRA.grib2/', dataDirOut='../result/tide/2018/')
    tf.setDuration(begin='2018-01-01', end='2018-12-31')
