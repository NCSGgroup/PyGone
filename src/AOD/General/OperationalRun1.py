...
def run_hybrid(BeginDate, EndDate):
    ...
    for day in daylist:
        ...
        '''produced by Cheung'''
        cspmean = np.mean(np.array(list(cs_sp.Cnm.values())),axis=0)
        sspmean = np.mean(np.array(list(cs_sp.Snm.values())),axis=0)
        cuppermean = np.mean(np.array(list(cs_upper.Cnm.values())),axis=0)
        suppermean = np.mean(np.array(list(cs_upper.Snm.values())),axis=0)

        # cmean = np.mean(np.array(list(cs_sp.Cnm.values()) + np.array(list(cs_upper.Cnm.values()))), axis=0)
        # smean = np.mean(np.array(list(cs_sp.Snm.values()) + np.array(list(cs_upper.Snm.values()))), axis=0)

        delta_cs_sp1 = copy.deepcopy(cs_sp)
        delta_cs_upper1 = copy.deepcopy(cs_upper)
        delta_cs1 = copy.deepcopy(delta_cs_sp1)
        ...
        for key in delta_cs_sp1.Cnm.keys():
            delta_cs_sp1.Cnm[key] -= cspmean
            delta_cs_sp1.Snm[key] -= sspmean
            delta_cs_upper1.Cnm[key] -= cuppermean
            delta_cs_upper1.Snm[key] -= suppermean
            delta_cs1.Cnm[key] = delta_cs_sp1.Cnm[key] + delta_cs_upper1.Cnm[key]
            delta_cs1.Snm[key] = delta_cs_sp1.Snm[key] + delta_cs_upper1.Snm[key]
        ...
        fm_spmean.setCS(delta_cs_sp1).AODstyle()
        fm_uppermean.setCS(delta_cs_upper1).AODstyle()
        fm_product.setCS(delta_cs1).AODstyle()
