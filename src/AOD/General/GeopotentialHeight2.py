...
class GeopotentialHeight:
    ...
    def __get_ph_levs(self, level):
        ...
        if level == self.__field.getLevel():
            return self.__lp[level - 1].self.__sp
        elif level == 0:
            return 0, self.__lp[level]
        else:
            return self.__lp[level - 1], self.__lp[level]
    ...
    def __compute_z_level(self, lev, t_level, q_level, z_h):
        ...
        tv_level = tv_level * Constants.Rd
    ...
    def produce_z(self):
        ...
        ghml = list(map(lambda x: x / Constants.g_wmo, z))

        # geopotential height on full model level
        ghfml = list(map(lambda x: x / Constants.g_wmo, zf))
