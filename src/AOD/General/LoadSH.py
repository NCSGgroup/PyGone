...
class Gif48(LoadSH):
    ...
    def __read(self, content: list):
        ...
        sigmaC = np.array(sigmaC).astype(np.float)
        sigmaS = np.array(sigmaS).astype(np.float)
        ...
        self._sigmaC[(l * (l + 1) / 2 + m).astype(np.int)] = sigmaC
        self._sigmaS[(l * (l + 1) / 2 + m).astype(np.int)] = sigmaS
